...
def show_cart(request):
    ...
    for p in cart:
        ...
        if amount < 30:
            ...
            shipping = 0
        taxamount = round(amount * taxpercent, 2)

    taxamount = taxamount + 0
    totalamount = round((amount + taxamount + shipping), 2)
...
class checkout(View):
    def get(self, request):
        ...
        for p in cart_items:
            ...
            taxamount = round(famount * taxpercent, 2)
            if famount < 30:
                shipping = 2.99
            else:
                shipping = 0
        ...
        data = {"amount": razoramount, "currency": "CAD", "receipt": "order_rcptid_12"}
...
def plus_cart(request):
    if request.method == 'GET':
        ...
        totalamount = round((amount + shipping + taxamount), 2)
        data = {
            'quantity': c.quantity,
            'amount': amount,
...
def remove_cart(request):
    if request.method == 'GET':
        ...
        taxamount = round(amount * taxpercent, 2)
        totalamount = amount + shipping + taxamount
        data = {
            'amount': amount,
            'totalamount': totalamount,
            'taxamount': taxamount,
